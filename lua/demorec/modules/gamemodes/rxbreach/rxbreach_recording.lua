function IsBreachRecordingEnabled()
    return config.ForcedModules["rxbreach"] == true
end

function ManageDemoRecording(action)
    if GetGlobalInt("demorec_enabled") == 0 or not IsBreachRecordingEnabled() then return end
...
net.Receive("PostStart", function()
